...
//        pseudo_optimized(file_velo_image_view_pcd[i], file_pseudo_velodyne_pcd[i], file_pseudo_optimized_pcd[i], file_pseudo_label[i], file_error[i]);
    ...
    * 5. generate datasets
    * ----------------------------------------------------*/
    string data = "/home/Data1/Datasets/KITTI/classify/data/";
    string label = "/home/Data1/Datasets/KITTI/classify/label/";
    if (access(data.c_str(), 0)) {
        cout << data << " is not existing" << endl;
        mkdir(data.c_str(), 0777);
    }
    if (access(label.c_str(), 0)) {
        cout << label << " is not existing" << endl;
        mkdir(label.c_str(), 0777);
    }
    vector<string> file_data;
    vector<string> file_label;
    for (int i = 0; i < indices.size(); i++) {
        file_data.push_back(pseudo_velodyne_bin + indices[i] + ".bin");
        file_label.push_back(pseudo_velodyne_pcd + indices[i] + ".pcd");
    }

//    cout << file_error[0] << endl;
//    int i = 100;
//    generate_datasets(file_error[i], file_velo_image_view_pcd[i], file_pseudo_velodyne_pcd[i], i);
...
//    training_datasets(file_error[i], file_velo_image_view_pcd[i], file_pseudo_velodyne_pcd[i], training_path, i);
//    hello();
